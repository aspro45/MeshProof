...
export default function SubmitReviewPage() {
  ...
  const valid = !!selected && claim.trim().length >= 40 && evidence.length > 0;

  const submit = async () => {
    if (!selected) return;
    const h = await run("Submit review", "submit_review", [
      selected.assetId,
      claim.trim(),
      evidence.map((item) => item.url),
      evidence.map((item) => item.digest),
    ]);
    if (h) { setDone(selected.assetId); setClaim(""); setEvidence([]); assets.reload(); }
  };
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_minmax(0,360px)]">
        <div className="panel space-y-4 p-4">
          ...
          <EvidenceBundleInput label="Immutable evidence bundle" items={evidence} onChange={setEvidence} max={6} />

          <div className="flex items-center justify-between">
            <span className="text-xs text-muted">{valid ? <span className="inline-flex items-center gap-1 text-verified"><FontAwesomeIcon icon={faCircleCheck} className="h-3 w-3" /> Ready</span> : "Select an asset, write a 40+ character claim, and add evidence with SHA-256."}</span>
            ...
            <>
              ...
              <div className="panel space-y-2 p-4">
                ...
                <div className="border-t border-line pt-2 text-[11px] text-muted">
                  <span className="label">Model SHA-256</span>
                  <span className="mono mt-1 block break-all text-text">{selected.modelDigest}</span>
                </div>
              </div>
            </>
          ) : (

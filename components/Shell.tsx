...
import {
  faCube, faFilePen, faScaleBalanced, faUserShield, faBoxesStacked, faBars, faXmark,
} from "@fortawesome/free-solid-svg-icons";
import { MeshProofLogo } from "./MeshProofLogo";
import { WalletConnect } from "./WalletConnect";
import { hasContract, CONTRACT } from "@/lib/meshproof";
import { CHAIN_ID, NETWORK_NAME } from "@/lib/studionet";
import { Hex } from "./ui";
...
export function Shell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden lg:grid lg:grid-cols-[256px_1fr]">
      <aside className={`fixed inset-y-0 left-0 z-40 w-[256px] border-r border-line bg-panel/95 backdrop-blur transition-transform lg:static lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="absolute bottom-0 w-full border-t border-line p-3 text-[11px] text-muted">
          <div className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-verified" /> GenLayer {NETWORK_NAME} · chain {CHAIN_ID}</div>
